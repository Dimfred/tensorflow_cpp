...
Status ReadImageFile(const string &filename, std::vector<Tensor>* out_tensors){
	...
	cout << "before decode image" << endl;
	auto decoder = root.WithOpName("file_decoder");
	cout << "after decode op" << endl;
	const int wanted_channels = 3;
	tensorflow::Output image_reader = tensorflow::ops::DecodeImage(decoder, output);
	cout << "after decode image" << endl;
